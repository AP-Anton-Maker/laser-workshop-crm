...
from bot.vk_client import get_vk_session
...
def init_vk():
    """Инициализировать VK сессию"""
    global vk, longpoll
    if vk is None:
        _, vk, longpoll = get_vk_session()
    return vk, longpoll

def send_vk_message(user_id, text, keyboard=None):
    vk_api, _ = init_vk()
    if vk_api:
        vk_api.messages.send(user_id=user_id, message=text, random_id=get_random_id(), keyboard=keyboard)
...
class Command(BaseCommand):
    ...
    def handle_message(self, event):
        ...
        vk_api, _ = init_vk()
        if not vk_api:
            logger.error("VK не настроен, пропускаем сообщение")
            return
